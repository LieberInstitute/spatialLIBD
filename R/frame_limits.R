...
#' }
#'
frame_limits <- function(spe, sampleid, image_id = "lowres", visium_grid = list(row_min = 0, row_max = 77, col_min = 0, col_max = 127, fiducial_vs_capture_edge = (8 - 6.5) * 1000 / 2 / 100)) {
    ...
    d$array_row_edge <- sapply(d$array_row, function(x) { min(abs(c(visium_grid$row_min, visium_grid$row_max) - x)) })
    d$array_col_edge <- sapply(d$array_col, function(x) { min(abs(c(visium_grid$col_min, visium_grid$col_max) - x)) })
    ...
    pxl_100um_col_then_row <- max(pxl_100um_compute(c("array_col", "array_row")))
    pxl_100um_row_then_col <- max(pxl_100um_compute(c("array_row", "array_col")))
    ...
    options(width = 160)
    do.call(rbind, list(
        d[d$pxl_row_in_fullres == min(d$pxl_row_in_fullres), c("array_col", "array_row", "pxl_col_in_fullres", "pxl_row_in_fullres", "array_row_edge", "array_col_edge", "array_min_edge")],
        d[d$pxl_row_in_fullres == max(d$pxl_row_in_fullres), c("array_col", "array_row", "pxl_col_in_fullres", "pxl_row_in_fullres", "array_row_edge", "array_col_edge", "array_min_edge")],
        d[d$pxl_col_in_fullres == min(d$pxl_col_in_fullres), c("array_col", "array_row", "pxl_col_in_fullres", "pxl_row_in_fullres", "array_row_edge", "array_col_edge", "array_min_edge")],
        d[d$pxl_col_in_fullres == max(d$pxl_col_in_fullres), c("array_col", "array_row", "pxl_col_in_fullres", "pxl_row_in_fullres", "array_row_edge", "array_col_edge", "array_min_edge")]
    ))
    ...
    frame_lims$y_max <- ifelse(frame_lims$y_max > nrow(img), nrpw(img), frame_lims$y_max)
