...
app_server <- function(input, output, session) {
    ...
    output$histology_plotly <- renderPlotly({
        ...
        d <- as.data.frame(cbind(colData(spe), spatialCoords(spe))[spe$sample_id == sampleid, ], optional = TRUE)
    ...
    observeEvent(input$update_manual_ann_gene, {
        ...
        if (!is.null(event.data)) {
            ## Prepare the data
            d <- as.data.frame(cbind(colData(spe), spatialCoords(spe))[spe$key %in% event.data$key, ], optional = TRUE)
