...
read10xVisiumAnalysis <- function(samples = "",
    ...
    clusters_all <- do.call(rbind, mapply(function(current_dir, current_sample) {
        ...
        clusters <- Reduce(function(...) merge(..., by = "barcode", all = TRUE), clusters_list)
        clusters$sample_id <- current_sample
        return(clusters)
    }, dir, sids, SIMPLIFY = FALSE, USE.NAMES = FALSE))


    projection_all <- mapply(function(current_dir, current_sample) {
        projection_files <-
            list.files(
                current_dir,
        ...
        names(projection_list) <- paste0("10x_", basename(dirname(dirname(projection_files))))
    ...
    cluster_cols <- which(!colnames(clusters_all) %in% c("barcode", "sample_id"))
