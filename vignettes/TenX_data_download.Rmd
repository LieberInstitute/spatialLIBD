...
```{bash, eval = FALSE}
$ cd /dcl02/lieber/ajaffe/SpatialTranscriptomics/
$ du -sh --apparent-size refdata-gex-GRCh38-2020-A/genes/genes.gtf
...
To facilitate reading in the data and preparing it to visualize it interactively using `spatialLIBD::run_app()`, we implemented `read10xVisiumWrapper()` which expands `SpatialExperiment::read10xVisium()` and performs the steps described in this vignette.
