...
The **querey** dataset will be the DLPFC single nucleus RNA-seq (snRNA-seq) data from `r Citep(bib[['tran2021']])`. 

We will compare the gene expression in the cell type populations of the **querey**
...
`spatialLIBD` contains many functions to compute `modeling_results` for the query sc/snRAN-seq or spatial data. 
...
## cellType is a factor - use character vector to avoid empty level bug 
sce.dlpfc.tran$cellType2 <- as.character(sce.dlpfc.tran$cellType)

## need ensemblID as rownames
rownames(sce.dlpfc.tran) <- rowData(sce.dlpfc.tran)$gene_id

## if empty levels after min_ncells
sce_modeling_results <- registration_wrapper(sce = sce.dlpfc.tran,
                                         var_registration = "cellType2",
                                         var_sample_id = "donor",
