...
bib <- c(
    ...
    SpatialExperiment = citation("SpatialExperiment"),
...
As of version 1.1.7, `r Biocpkg('spatialLIBD')` supports the `VisiumExperiment` class from `r Biocpkg('SpatialExperiment')`. The functions `sce_image_gene_p()`, `sce_image_gene()`, `sce_image_grid()`, `sce_image_grid_gene()`, `sce_image_clus()`, `sce_image_clus_p()`, `geom_spatial()` now work with `VisiumExperiment` objects thanks to the new function `read_image()`. We
provide now the function `sce_to_ve()` for converting our data to the `VisiumExperiment` class. This work was done by [Brenda Pardo](https://twitter.com/PardoBree) and Leonardo. 
