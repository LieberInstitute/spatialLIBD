...
```
Using the above enrichment table, we can then visualize the odds ratios on a dot plot as shown below. Note that we use the thresholded p-values at `-log10(p) = 12` for visualization purposes and only show the odds ratios for `-log10(p) > 3` by default.
